...
$(document).ready(function () {
	function initializeCarousel(carouselId, apiUrl) {
		$.ajax({
			url: apiUrl,
			method: "GET",
			success: function(response) {
				...
				$(carouselId).empty().append(allTutos);

				// Initialize Slick Carousel
				$(`${carouselId} .carousel-inner`).slick({
					slidesToShow: 4,
					slidesToScroll: 1,
					arrows: true,
	...
	initializeCarousel('#carouselExampleControls2', "https://smileschool-api.hbtn.info/popular-tutorials");
	initializeCarousel('#carouselExampleControls3', "https://smileschool-api.hbtn.info/latest-videos");
